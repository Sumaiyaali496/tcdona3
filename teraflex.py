...
class TFlex:
    ...
    def __init__(self, ip_address='10.10.10.92'):
        ...
        if check_patch_owners([('tf1','tf1'),
                            ('tf2','tf2'),
                            ('tf3','tf3'),
                            ('tf4','tf4')]): #We need to map individual line ports to patches and then configure check_patch_list for each set line_port method
    ...
    def change_configuration(
        ...
        if self._config[line_port]["modulation"] != modulation:
            self.__set_admin_maintenance(line_port + "/" + logical_interface)
            self.set_interface_modulation(line_port, modulation)
            self.__remove_admin_maintenance(line_port + "/" + logical_interface)
            sleep_counter = 150
        ...
        self.set_power_and_frequency(
            line_port=line_port, power=target_power, frequency=central_frequency
        )
